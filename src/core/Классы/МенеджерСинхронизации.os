...
Процедура СинхронизироватьХранилищеКонфигурацийСГит(Знач КаталогРабочейКопии, 
		...
		Знач ПараметрыДоступаКХранилищу = Неопределено,
		Знач КаталогПлагинов = Неопределено) Экспорт

	Если КаталогПлагинов <> Неопределено Тогда
		//Грузим плагины только 1 раз
		Если ЗагруженныеПлагины = Неопределено Тогда
			ЗагруженныеПлагины = Новый Соответствие;

			ФайлКаталогаПлагинов = Новый Файл(КаталогПлагинов);
			Если ФайлКаталогаПлагинов.Существует() и ФайлКаталогаПлагинов.ЭтоКаталог() Тогда
				ФайлыПлагинов = НайтиФайлы(ФайлКаталогаПлагинов.ПолноеИмя, "*.os");
				Для каждого ТекущийПлагин Из ФайлыПлагинов Цикл
					Лог.Отладка("Загружаю плагин " + ТекущийПлагин.Имя);
					ЗагруженныеПлагины.Вставить(ТекущийПлагин.Имя, ЗагрузитьСценарий(ТекущийПлагин.ПолноеИмя));
				КонецЦикла;
			Иначе
				ВызватьИсключение "Некорректный путь к плагинам: " + ФайлКаталогаПлагинов.ПолноеИмя;
			КонецЕсли;
		КонецЕсли;
	Иначе
		ЗагруженныеПлагины = Новый Соответствие;
	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Попытка
				...
				СтандартнаяОбработкаГлоб = Истина;
				Для каждого ТекущийПлагин Из ЗагруженныеПлагины Цикл
					СтандартнаяОбработкаЛок = Истина;
					Лог.Отладка("Выполнение [ПередВыполнитьКоммитГит] для плагина " + ТекущийПлагин.Ключ);
					...
					ТекущийПлагин.Значение.ПередВыполнитьКоммитГит(ПараметрыОбработки, СтандартнаяОбработкаЛок);

					//Считаем, что если хотя бы один плагин сбросил стандартную обработку, то дальше не идем
					СтандартнаяОбработкаГлоб = СтандартнаяОбработкаГлоб и СтандартнаяОбработкаЛок;
				КонецЦикла;

				Если СтандартнаяОбработкаГлоб Тогда
